...
export default function DashboardContact() {
  ...
  const [allFormtData, setAllFormData] = useState([]);
  const {
    isFetching,
    data: getAllForm,
    isLoading,
  ...
  ] = api.adminApis.useDeleteFormSubmissionMutation();
  ...
  const initiateDelete = (item) => {
    setItemToDelete({ id: item?._id, name: `${item?.fullName}` });
    setIsDeleteModalOpen(true);
  };
  const handleDelete = (id) => {
    deleteTrigger(id);
    setAllFormData(allFormtData.filter((item) => item._id !== id));
  };
  ...
  const { items } = usePagination({
    count: Math.ceil(allFormtData?.length / rowsPerPage),
    onChange: (event, page) => handleChange(event, page),
    page: page,
  });
  const filterData = (allFormtData, searchQuery) => {
    if (!searchQuery) return allFormtData;
    return allFormtData.filter((user) => {
      const searchString =
            ...
            <TableContainer component={Paper}>
              <Table className="w-full" cellPadding={14}>
                ...
                {isFetching ? (
                  ...
                  <TableBody>
                    {filterData(allFormtData, searchData.name)
